    ...
    public partial class Reflector
    {
        public static class Error
        {
            ...
            public static string NotFoundAsset(string? assetPath, string? assetGuid)
                => $"[Error] Asset not found. Path: {assetPath ?? "null"}. GUID: {assetGuid ?? "null"}.";

            public static string NotAllowedToModifyAssetInPackages(string? assetPath)
                => $"[Error] Not allowed to modify asset in '/Packages' folder. Path: {assetPath ?? "null"}.";
            ...
            public static string NotFoundType(string? typeFullName)
                => $"[Error] Type '{typeFullName ?? "null"}' not found.";
            ...
            public static string TypeMismatch(string? expectedType, string? objType)
                => $"[Error] Type mismatch between '{expectedType ?? "null"}' (expected) and '{objType ?? "null"}'.";
            ...
            public static string InvalidInstanceID(Type holderType, string? fieldName)
                => $"[Error] Invalid instanceID '{fieldName ?? "null"}' for '{holderType.FullName}'. It should be a valid field name.";
            public static string InvalidComponentPropertyType(SerializedMember serializedProperty, PropertyInfo propertyInfo)
                => $"[Error] Invalid component property type '{serializedProperty.typeName ?? "null"}' for '{propertyInfo.Name}'. Expected '{propertyInfo.PropertyType.FullName}'.";
            public static string InvalidComponentFieldType(SerializedMember serializedProperty, FieldInfo propertyInfo)
                => $"[Error] Invalid component property type '{serializedProperty.typeName ?? "null"}' for '{propertyInfo.Name}'. Expected '{propertyInfo.FieldType.FullName}'.";

            public static string NotSupportedInRuntime(Type type)
                => $"[Error] Type '{type.FullName ?? "null"}' is not supported in runtime for now.";
