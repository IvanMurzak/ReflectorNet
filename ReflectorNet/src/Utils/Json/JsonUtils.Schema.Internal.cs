...
namespace com.IvanMurzak.ReflectorNet.Utils
{
    public static partial class JsonUtils
    {
        ...
        {
            static List<JsonNode> FindAllProperties(JsonNode node, string fieldName)
            {
            ...
            static void PostprocessFields(JsonNode? node)
            {
                if (node == null)
                    return;
            ...
            /// </summary>
            static JsonNode GenerateSchemaFromType(Type type)
            {
                ...
                if (type.IsArray)
                {
                    ...
                    {
                        [Type] = Array,
                        [Items] = elementType != null ? GetSchema(elementType, justRef: false) : new JsonObject()
                    };
                }
                ...
                if (type.IsGenericType && TypeUtils.IsIEnumerable(type))
                {
                    ...
                    if (itemType != null)
                    {
                        ...
                        {
                            [Type] = Array,
                            [Items] = GetSchema(itemType, justRef: false)
                        };
                    }
                }

                // Handle regular objects by introspecting their fields and properties
                var reflector = Reflector.Instance ?? new Reflector();
                ...
                if (fields != null)
                {
                    ...
                    {
                        var fieldSchema = GetSchema(field.FieldType, justRef: false);
                ...
                if (props != null)
                {
                    ...
                    {
                        var propSchema = GetSchema(prop.PropertyType, justRef: false);
            ...
            /// </summary>
            static JsonNode GeneratePrimitiveSchema(Type type)
            {
