        ...
        public static class Schema
        {
            ...
            public static JsonNode? GetArgumentsSchema(MethodInfo method, bool justRef = false)
            {
                ...
                foreach (var parameter in parameters)
                {
                    ...
                    {
                        var typeId = GetTypeId(parameter.ParameterType);
            ...
            static string GetTypeIdRecursive(Type type)
            {
                ...
                if (type == typeof(string))
                    return type.FullName ?? throw new InvalidOperationException($"Type '{type}' does not have a full name.");
                ...
                if (type.IsGenericType)
                {
                    ...
                    return $"{genericTypeName}<{string.Join(",", type.GetGenericArguments().Select(t => t.FullName))}>";
                }

                return type.FullName ?? throw new InvalidOperationException($"Type '{type}' does not have a full name.");
            }

            private static Type? GetArrayLikeItemType(Type type)
            {
